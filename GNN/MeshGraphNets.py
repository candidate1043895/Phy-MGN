...
class MeshGraphNets(GNN):
    ...
    def update_function(
        self, mgn_output_np, current_state=None, previous_state=None, source_data=None, output_type=None
    ):
        """
        ...
        #print('output type kwargs',output_type) 
        ...
        with no_grad():
            if self.output_type == "acceleration":
                ...
                #next_state = mgn_output_np.copy_()  #I changed it on 26/06/2024, from .copy() to .copy_()
                next_state = torch.clone(mgn_output_np)
             
            # I added these on 15/07, to rollout on two quantities at once 
            elif self.output_type == "acceleration & pressure":
                assert current_state is not None
                assert previous_state is not None
                next_state = np.empty_like(current_state)
                next_state[:, :2] = 2 * current_state[:, :2] - previous_state[:, :2] + mgn_output_np[:,:2]
                next_state[:,2] = torch.clone(mgn_output_np[:,2])            
                ...
                next_state[:, 2] = current_state[:,2] + mgn_output_np[:,2]
                next_state[:,3] = mgn_output_np[:,3].clone()
                next_state[:, 5] = current_state[:,4] + mgn_output_np[:,5]
                next_state[:, 6] = current_state[:,5] + mgn_output_np[:,6]
                
            elif output_type == "dens" or output_type =="pres":
                next_state =  mgn_output_np.clone()
            ...
            if type(source_data) is dict:
                ...
                next_state[source_data[0]] = source_data[1].to(next_state.dtype) #added on 29/09/2024 keep dtype consistance between source and destination 
            else:
    ...
    def rollout_step(
        ...
        with no_grad():
            ...
            # if self.use_adaptive_mesh:
            #     pass
            #print(np.shape(mgn_output))
            
            #print('output type', update_func_kwargs['output_type'])
            # update state
            next_state = update_func(
                ...
                output_type=update_func_kwargs['output_type'],
            )
    ...
    def rollout(
        ...
        for i in iter_obj:
            ...
            if type(source_data) is dict:
                ...
                max_idx = source_data[1].shape[0] - 1  # last available time index
                safe_idx = min(i, max_idx)
                source_data_i = (
                    source_data[0],
                    tensor(source_data[1][safe_idx, :, :], device=device),
                )
