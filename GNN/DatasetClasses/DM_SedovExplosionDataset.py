...
from GNN.utils.data_utils import add_edges_to_nodes, quads_to_edges
from GNN.DatasetClasses.base import MGNDataset


class DM_SedovExplosionDataset(MGNDataset):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        dataset_params = model_config.DM_SEDOV_EXPLOSION_PARAMS
        # prepare train / test sets, Sedov explosion
        if train_set:
        ...
        print('ouput type', self.output_type)
        ...
        self.pres_history = model_config.get_pres_history()
    ...
    def update_function(
        ...
        with no_grad():
            if output_type == "acceleration":
                ...
                assert current_state is not None
                assert previous_state is not None
                next_state = torch.empty_like(current_state)
                next_state[:, :2] = 2 * current_state[:, :2] - previous_state[:, :2] + mgn_output_np[:,:2]
                next_state[:,2] = mgn_output_np[:,2].clone()

            # rollout velocity and pressure at the same time ( static mesh )
            elif output_type == "velocity & pressure":
                assert current_state is not None
                next_state = torch.empty_like(current_state)
                next_state[:, :2] = current_state[:,:2] + mgn_output_np[:,:2]
                next_state[:,2] = mgn_output_np[:,2].clone()
                ...
                next_state[:, :2] = current_state[:,:2] + mgn_output_np[:,:2]
                next_state[:, 2] = current_state[:,2] + mgn_output_np[:,2]
                next_state[:,3] = mgn_output_np[:,3].clone()
            else:  # state
    ...
    def process(self):
        ...
        def fname_to_ints(fname):
            ...
            fname = fname.replace("SedovExplosion_", "").replace(".pt", "")
            fname = (
        ...
        if osp.exists(self.processed_dir) and not self.force_preprocess:
            ...
            for fname in processed_file_names:
                ...
                data_filename = self.direc + "sample_%s.hdf5" % (traj_idx)
                data_sample = h5py.File(data_filename, 'r')
                expected_timesteps = (
            ...
            for traj_idx in self.sample_idx:
                ...
                processed_file_name = f"SedovExplosion_sim{traj_idx}_nsteps{n_timesteps}_npatches{n_patches}.pt"
                processed_path = osp.join(self.processed_dir, processed_file_name)

                while True:
                    try:
                        # Attempt to write the data to a pickle file
                        with open(processed_path, "wb") as f:
                            pickle.dump(patches, f)
                        # Attempt to read the data back from the pickle file
                        with open(processed_path, "rb") as f:
                            test_loaded_data = pickle.load(f)
                        # If successful, break the loop
                        break
                    except:
                        # If there's an error, print the error and retry
                        print(f"Invalid .pt file. Retrying...")
    ...
    def __getitem__(self, idx):
        ...
        graph = self.get_graph_for_simulation(traj_idx)
        data = graph.output
        #print('data shape', data.shape, 'traj_idx', traj_idx)
        ...
        node_data, outputs = get_sample(
            ...
            pres_history_flag=self.pres_history,
            noise_sd=self.noise,
        ...
        if self.pres_history:
            patch_ground_truth = patch_outputs[:,:4]
            patch_pressure = patch_outputs[:,4].reshape(-1, 1)
            patch_extra = patch_outputs[:, 5:]
        elif not self.pres_history:
            patch_ground_truth = patch_outputs
            patch_pressure = None
            patch_extra = None
            
        patch_graph = Data(
            x=patch_node_data,
            y=patch_ground_truth,
            pressure=patch_pressure, 
            extra = patch_extra, 
    ...
    def rollout_info(self, sim_num, rollout_start_idx, window_length, sources):
        ...
        cells = self.get_cells_for_simulation(sim_num)
    ...
    def get_cells_for_simulation(self, traj_idx):
        '''
        Inputs:
        traj_idx: the ID of a simulation

        Returns:
        Data object representing cells of simulation mesh
        '''
        ...
        data_sample = h5py.File(data_filename, 'r')
        # cells
        cells = data_sample["cells"][0]
        #No adaptive mesh yet, so static cells (for now)
    ...
    def get_graph_for_simulation(self, traj_idx):
        ...
        data_filename = self.direc + "sample_%s.hdf5" % (traj_idx)
        data_sample = h5py.File(data_filename, 'r')
        #print('get_sample output type', self.output_type)
        if self.output_type == "velocity":
            data = data_sample["velocity"]
            
        elif self.output_type == "dens" or self.output_type == "dens_dt":
        
            #data = data_sample["density"]
            data = data_sample["density"]
            #print('density  data shape', data.shape)
        elif self.output_type == "pres" or self.output_type == "pres_dt":
            data = data_sample["pressure"][20:60, :, :]
            #print(' pressure data shape', data.shape)
        elif self.output_type == "velocity & pressure":
            data = np.concatenate((data_sample["velocity"], data_sample["pressure"]), axis=-1)
     
        elif self.output_type == "velocity & density & pressure":
            
            data = np.concatenate((data_sample["velocity"], data_sample["density"], data_sample["pressure"]), axis=-1)
            #print('velocity & density & pressure data shape', data.shape)
        else:
            raise ValueError("output type not implemented")
        ...
        # Stationary mesh coordinates only, need modufication when using AMR (adaptive mesh refinement)
        ...
        for i, graph_type in enumerate(self.graph_types):
            ...
            if graph_type == "quads":
                ...
                edge_index = torch.tensor(quads_to_edges(data_sample["cells"][0]))
