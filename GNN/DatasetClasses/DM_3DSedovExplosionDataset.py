...
from GNN.utils.data_utils import add_edges_to_nodes, cubes_to_edges
from GNN.DatasetClasses.base import MGNDataset


class DM_3DSedovExplosionDataset(MGNDataset):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        dataset_params = model_config.DM_3DSEDOV_EXPLOSION_PARAMS
        # prepare train / test sets, 3D Sedov explosion
        if train_set:
    ...
    def process(self):
        ...
        def fname_to_ints(fname):
            ...
            fname = fname.replace("3DSedovExplosion_", "").replace(".pt", "")
            fname = (
        ...
        if osp.exists(self.processed_dir) and not self.force_preprocess:
            ...
            for traj_idx in self.sample_idx:
                ...
                processed_file_name = f"3DSedovExplosion_sim{traj_idx}_nsteps{n_timesteps}_npatches{n_patches}.pt"
    ...
    def get_graph_for_simulation(self, traj_idx):
        ...
        for i, graph_type in enumerate(self.graph_types):
            ...
            if graph_type == "cubes":
                ...
                edge_index = torch.tensor(cubes_to_edges(data_sample["cells"][0]))    # Need to write cubes to edges function
