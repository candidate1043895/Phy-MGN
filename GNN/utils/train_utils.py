...
from torch import no_grad, cat, save, load, distributed, tensor, is_tensor, nn
...
def train_epoch(
    ...
    for i, data_list in enumerate(tqdm_itr):
        with autocast(amp_device, enabled=amp):
            ...
            pde_loss = loss_function(data_list, m, parallel, mask, train)
            
            loss_cont = pde_loss['continuity']
            loss_ideal = pde_loss['ideal_gas']
            loss_mx = pde_loss['momentum_x']
            loss_my = pde_loss['momentum_y']
            loss_data = pde_loss['data']
            
            use_exp_weight = m_config.get_use_exp_weight()
            if not use_exp_weight:
                loss_cont_w = m_config.get_cont_weight()
                loss_ideal_w = m_config.get_ideal_weight()
                loss_mx_w = m_config.get_mx_weight()
                loss_my_w = m_config.get_my_weight()
                loss_data_w = m_config.get_data_weight()
                
            elif use_exp_weight:
                print('using exp weight')
                wi = m_config.get_wi()
                wf = m_config.get_wf()
                rate = m_config.get_weight_rate()
                
                pde_weight = get_pde_weight(epoch, rate, wi, wf)
                print('pde weight')
                
                loss_cont_w = pde_weight
                loss_ideal_w = m_config.get_ideal_weight()
                loss_mx_w = pde_weight
                loss_my_w = pde_weight
                loss_data_w = m_config.get_data_weight()
           
            
            loss =( loss_data_w * loss_data 
                   + loss_cont_w * loss_cont 
                   + loss_ideal_w * loss_ideal
                   + loss_mx_w * loss_mx 
                   + loss_my_w * loss_my )
            
            #loss_pde = (loss_cont_w * loss_cont 
            #       + loss_ideal_w * loss_ideal
            #       + loss_mx_w * loss_mx 
            #       + loss_my_w * loss_my )
        ...
        # record grad from data and pde loss------------------------------------------------------
        #optimizer.zero_grad()

        # 2️⃣ Data loss gradients
        #loss_data.backward(retain_graph=True)
        #grads_data = {name: p.grad.detach().cpu() for name, p in m.named_parameters() if p.grad is not None}
        #optimizer.zero_grad()

        # 3️⃣ PDE loss gradients
        #loss_pde.backward(retain_graph=True)
        #grads_pde = {name: p.grad.detach().cpu() for name, p in m.named_parameters() if p.grad is not None}
        #optimizer.zero_grad()

        # 4️⃣ Total loss gradients (optional)
        #loss.backward()
        #grads_total = {name: p.grad.detach().cpu() for name, p in m.named_parameters() if p.grad is not None}

        # 5️⃣ Save all into one .pt file        
        #save(
        #    {
        #        "epoch": epoch,
        ...
        #    },
        #    f"/work4/clf/scarf1271/GNN/MeshGraphNet_all_in_one/training_scripts/gradients_global_step_{global_step}.pt"
        #)

        #print(f"✅ Saved gradient tensors for epoch {epoch} to grad_logs/gradients_global_step_{global_step}.pt")
        # ------------------------------------------------------------------------------------------------------------
        ...
        loss_pde_mom = loss_mx_w * loss_mx + loss_my_w * loss_my
        loss_pde_cont = loss_cont_w * loss_cont
        loss_pde_ideal = loss_ideal_w * loss_ideal
        
        loss_values = {
            'pde': loss_pde,
            'pde_mom': loss_pde_mom,
            'pde_cont': loss_pde_cont,
            'pde_ideal': loss_pde_ideal,
            'data': loss_data,
            'total': loss,
        }
        for key in total_losses:
            total_losses[key] += loss_values[key].item()
        ...
        if global_step % tb_rate == 0:
            ...
            tqdm_itr.set_postfix_str("Train loss: %f" % (total_losses['total'] / (i + 1)))
        ...
        if (elapsed > time_limit_hours or global_step >= lim_steps) and (
            ...
            for key in total_losses:
                total_losses[key] = total_losses[key] / (i + 1)
                
            if ddp:
                
                for key in total_losses:
                    t = tensor(total_losses[key], device=device)
                    distributed.all_reduce(t)   # does sum reduction by default...
                    total_losses[key] = t.item() / env.world_size  # ...divide by world size to get mean loss
                
            if tb_writer is not None:
                tb_writer.add_scalar("Loss/train_epoch", scalar_value=total_losses['total'], global_step=epoch)
                tb_writer.add_scalar("PDELoss/train_epoch", scalar_value=total_losses['pde'], global_step=epoch)
                tb_writer.add_scalar("PDELoss_momentum/train_epoch", scalar_value=total_losses['pde_mom'], global_step=epoch)
                tb_writer.add_scalar("PDELoss_continuity/train_epoch", scalar_value=total_losses['pde_cont'], global_step=epoch)
                tb_writer.add_scalar("PDELoss_ideal_gas_law/train_epoch", scalar_value=total_losses['pde_ideal'], global_step=epoch)
                tb_writer.add_scalar("DataDrivenLoss/train_epoch", scalar_value=total_losses['data'], global_step=epoch)

                
            return total_losses, global_step, sample_wise_losses


    for key in total_losses:
            total_losses[key] = total_losses[key] / len(train_loader)
    ...
    if ddp:
        ...
        for key in total_losses:
            t = tensor(total_losses[key], device=device)
            distributed.all_reduce(t)   # does sum reduction by default...
            total_losses[key] = t.item() / env.world_size
    if tb_writer is not None:
        tb_writer.add_scalar("Loss/train_epoch", scalar_value=total_losses['total'], global_step=epoch)
        tb_writer.add_scalar("PDELoss/train_epoch", scalar_value=total_losses['pde'], global_step=epoch)
        tb_writer.add_scalar("PDELoss_momentum/train_epoch", scalar_value=total_losses['pde_mom'], global_step=epoch)
...
def train_epoch_LBFGS(
    ...
    for i, data_list in enumerate(tqdm_itr):
        ...
        print('back prop finished')
        total_loss += loss.item()
        running_loss += loss.item()

        # gather sample-wise losses for possible selective backprop during the main loop
        
        sample_wise_losses = 0
        ...
        if (elapsed > time_limit_hours or global_step >= lim_steps) and (
            ...
            if tb_writer is not None:
                tb_writer.add_scalar("Loss/train_epoch", scalar_value=total_loss, global_step=epoch)
        ...
        if elapsed > time_limit_hours or (i + 1) > sample_limit / sample_limit_divisor:
            ...
            if ddp:
                ...
                for key in total_losses:
                    ...
                    total_losses[key] = t.item() / env.world_size
            if tb_writer is not None:
                tb_writer.add_scalar("Loss/val_epoch", scalar_value=total_losses['total'], global_step=epoch)
                tb_writer.add_scalar("PDELoss/val_epoch", scalar_value=total_losses['pde'], global_step=epoch)
                tb_writer.add_scalar("PDELoss_momentum/val_epoch", scalar_value=total_losses['pde_mom'], global_step=epoch)
                tb_writer.add_scalar("PDELoss_continuity/val_epoch", scalar_value=total_losses['pde_cont'], global_step=epoch)
                tb_writer.add_scalar("PDELoss_ideal_gas_law/val_epoch", scalar_value=total_losses['pde_ideal'], global_step=epoch)
                tb_writer.add_scalar("DataDrivenLoss/val_epoch", scalar_value=total_losses['data'], global_step=epoch)
            return total_losses

    
        
    for key in total_losses:
        total_losses[key] = total_losses[key] / len(val_loader)
...
def get_lr_scheduler(model_config, train_loader, optimizer):
    ...
    if scheduler == "ExpLR":
        ...
        return ReduceLROnPlateau(optimizer, factor =scheduler_params["factor"], patience=scheduler_params["patience"])
...
def get_loss_function(model, m_config):
    
    function_name = model.get_loss_function_name()
    if function_name == "PINNLoss":
        return PINNLoss(model.device, m_config)
    else:
        return nn.MSELoss()
        
def get_pde_weight(epoch, rate, w0, wT):
    factor = (wT / w0) ** (epoch * rate)
    return w0 * factor

def pinn_loss(data_list, m_config, model, parallel, mask, device):


    loss_fn = PINNLoss(device, m_config)
    loss = loss_fn(data_list, model, parallel, mask)
    return loss    
