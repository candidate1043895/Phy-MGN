...
def build_parser_full_domains():
    ...
    parser.add_argument(
        "--GT_limit",
        action="store_true",
        help="Set color scale limit using mx and min from ground truth data",
    )
    return parser
...
def main_full_domains(args):
    ...
    output_dir = f"{m.test_output_dir}/{m.expt_name}"
    output_type = m.output_type
    already_ran = False
    if args.rollout_num is not None:
        if os.path.exists(
            f"{output_dir}/rollout_{dataset_str}_{args.rollout_num}.pk"
        ):
            already_ran = True
            print('rollout number {} already exists, dont want to do it again lol'.format(args.rollout_num))
    if not already_ran:
    ...
    if args.viz_rollout_num is not None:
        #we want to visualise the same sample we rollout
        assert args.viz_rollout_num == args.rollout_num

        own_output_dir = f'{output_dir}/{dataset_str}_output_{args.viz_rollout_num}{"_GT" if args.GT else ""}{"_GT_limit" if args.GT_limit else ""}_{output_type}'
        source_file = f"{output_dir}/rollout_{dataset_str}_{args.rollout_num}.pk"
        source_file_GT = f"{output_dir}/rollout_{dataset_str}_{args.rollout_num}_GT.pk"
        if not os.path.exists(own_output_dir):
            os.makedirs(own_output_dir)
            destination_file = os.path.join(own_output_dir, os.path.basename(source_file))
            shutil.copy2(source_file, destination_file)
            destination_file_GT = os.path.join(own_output_dir, os.path.basename(source_file_GT))
            shutil.copy2(source_file_GT, destination_file_GT)
            print(f'{dataset_str}_output_{args.viz_rollout_num}{"_GT" if args.GT else ""}{"_GT_limit" if args.GT_limit else ""}_{output_type}  made, files copied') 
        #classify if rollout for prediction or ground truth
        if args.GT :
        ...
        loaded = pickle.load(
            open(
                f'{own_output_dir}/rollout_{dataset_str}_{args.viz_rollout_num}{"_GT" if args.GT else ""}.pk',
                "rb",
            )
        )
        loaded_GT = pickle.load(
            open(
                f'{own_output_dir}/rollout_{dataset_str}_{args.viz_rollout_num}_GT.pk',
                "rb",
            )
        )
        data = loaded["pred"]
        data_GT = loaded_GT["pred"]
        graph = pickle.load(
            open(
                f'{own_output_dir}/rollout_{dataset_str}_{args.viz_rollout_num}.pk',
                "rb",
            )
        )["coords"]
        graph = matplotlib.tri.Triangulation(graph[:, 0], graph[:, 1])
        print(f"Saving visualization to {own_output_dir}")
        
        if args.GT_limit:
            limits = (np.min(data_GT, axis=(0, 1)), np.max(data_GT, axis=(0, 1)))
        else:
            limits = None
        plot_utils.plot_rollout_separate(
            ...
            output_type,
            classify,
            save_dir=f"{own_output_dir}",
            outname=f'rollout_{dataset_str}_{args.viz_rollout_num}{"_GT" if args.GT else ""}{"_GT_limit" if args.GT_limit else ""}.mp4',
            limits = limits, 
        )
...
def minimal_rollout(dataset, model, i, m, device):
    ...
    print('steps', steps)
    print('d["rollout_iterations"]', d["rollout_iterations"])
...
def run_and_save_rollout(args, m):
    ...
    dataset_str = "train" if not args.test_set else "test"
    print('test set flag', args.test_set)
    ...
    print('output_dim', dataset.output_dim)
    ...
    model_filename = m.get_model_filename()
    print("loaded model", model_filename)
    model = load_model(checkpoint_dir, model_filename=model_filename, model=model)
    ...
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    print('number of rollour sims', dataset.num_sims)
    for i in dataset.sample_idx:
        ...
        print('output type rollout', m.get_output_type())
        print('input type rollout', d["input_type"])
        print('d["rollout_iterations"]', d["rollout_iterations"])
        rollout_output = {
        ...
        with open(outfile, "wb") as f:
            ...
            #    d["feature_size"],
            #), rollout_output["pred"].shape
            print('GT data shape', rollout_output["pred"].shape)
            pickle.dump(rollout_output, f)
