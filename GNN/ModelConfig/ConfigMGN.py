...
class ModelConfig:
    def __init__(self, config_file):
        ...
        self.input_type = (
            "velocity" if "input_type" not in c_data else c_data["input_type"]
        )
        ...
        m_data = config["MODEL"]
        self.random_seed = (
            None if "random_seed" not in m_data else m_data.getint("random_seed")
        )
        self.model_arch = (
        ...
        self.mgn_layers = 2 if "mgn_layers" not in m_data else m_data.getint("mgn_layers")
        self.mgn_layers_processor = 2 if "mgn_layers_processor" not in m_data else m_data.getint("mgn_layers_processor")
        self.mlp_norm_type = (
        ...
        self.switch_epoch = 10000 if "switch_epoch" not in t_data else int(t_data.getfloat("switch_epoch"))
        self.switch_epoch_flag = False if "switch_epoch_flag" not in t_data else t_data.getboolean("switch_epoch_flag")
        ...
        self.use_alw = (
            False if "use_alw" not in t_data else t_data.getboolean("use_alw")
        )
        self.log_alpha = float(math.log(2)) if "alpha" not in t_data else float(math.log(t_data.getfloat("alpha")))
        self.log_beta = float(math.log(2)) if "beta" not in t_data else float(math.log(t_data.getfloat("beta")))
        # pinn loss--------------- 
        self.PDE = (
            "Euler_compress" if "PDE" not in t_data else t_data["PDE"]
        )
        self.forward_method = (
            "boundary_mask" if "forward_method" not in t_data else t_data["forward_method"]
        )
        self.grad_method = (
            "finite_difference" if "grad_method" not in t_data else t_data["grad_method"]
        ) 
        self.pres_history = (
                False if "pres_history" not in t_data else t_data.getboolean("pres_history")
        )
        self.fd_dx = (
            0.0156 if "fd_dx" not in t_data else t_data.getfloat("fd_dx")
        ) 
        self.dt = (
            0.005 if "dt" not in t_data else t_data.getfloat("dt")
        )
        self.xsize = (
            64 if "xsize" not in t_data else t_data.getint("xsize")
        )
        self.ysize = (
            64 if "ysize" not in t_data else t_data.getint("ysize")
        )
        self.full_xsize = (
            64 if "xsize" not in t_data else t_data.getint("full_xsize")
        )
        self.full_ysize = (
            64 if "ysize" not in t_data else t_data.getint("full_ysize")
        )
        self.cont_weight = (
            None if "cont_weight" not in t_data else t_data.getfloat("cont_weight")
        )
        self.ideal_weight = (
            None if "ideal_weight" not in t_data else t_data.getfloat("ideal_weight")
        )
        self.mx_weight = (
            None if "mx_weight" not in t_data else t_data.getfloat("mx_weight")
        )
        self.my_weight = (
            None if "my_weight" not in t_data else t_data.getfloat("my_weight")
        )
        self.data_weight = (
            None if "data_weight" not in t_data else t_data.getfloat("data_weight")
        )
        # pde loss weight
        self.use_exp_weight = (
            False if "use_exp_weight" not in t_data else t_data.getboolean("use_exp_weight")
        )
        self.wi = (
            None if "wi" not in t_data else t_data.getfloat("wi")
        )
        self.wf = (
            None if "wf" not in t_data else t_data.getfloat("wf")
        )
        self.weight_rate = (
            None if "rate" not in t_data else t_data.getfloat("rate")
        )
        #------------------------------
        self.optimiser = (
            "AdamW" if "optimiser" not in t_data else t_data["optimiser"]
        )
        self.use_amp = (
        ...
        self.home_dir_name = (
            None 
            if "home_dir" not in t_data else t_data["home_dir"]
        )
        ...
        self.model_filename = (
            "best_val_model.pt"
            if "model_filename" not in t_data
            else t_data["model_filename"]
        )
        # logs
        self.log_dir = (
        ...
        self.world_size = (
            ...
            #Note that FZ changed 'c_data' to 't_data' below:
        self.data_num_workers = (
            0 if not "data_num_workers" in t_data else t_data.getint("data_num_workers")
        )
        ...
        self.rollout_iterations = (
            100
            if "rollout_iterations" not in c_test
            else c_test.getint("rollout_iterations")
        )
    ...
    def get_home_dir_name(self):
        return self.home_dir_name
    ...
    def get_mgn_layers(self):
        return self.mgn_layers
        
    def get_mgn_layers_processor(self):
        return self.mgn_layers_processor
    ...
    def get_switch_epoch_flag(self):
        return self.switch_epoch_flag
    ...
    def get_pinn_forward_method(self):
        return self.forward_method

    def get_grad_method(self):
        return self.grad_method
        
    def get_pres_history(self):
       return self.pres_history
    ...
    def get_full_xsize(self):
        return self.full_xsize

    def get_full_ysize(self):
        return self.full_ysize
        
    def get_cont_weight(self):
        return self.cont_weight

    def get_ideal_weight(self):
        return self.ideal_weight

    def get_mx_weight(self):
        return self.mx_weight
    ...
    def get_use_exp_weight(self):
        return self.use_exp_weight
    ...
    def get_rollout_iterations(self):
        return self.rollout_iterations
